...
export default class Login5Client {
    ...
    async refresh() {
        if (!this.refreshCredentials) throw new Error('Cannot refresh token if not logged in')
	...
	async #flow(credentials: Login5Credentials) {
        ...
        if (response.challenges) {
            let loginContext = response.loginContext
            let challenge = response.challenges.challenges[0].hashcash
    
            let solution = await this.solveHashcash(loginContext, challenge.prefix, challenge.length)
    ...
    async solveHashcash(loginContext: Buffer, prefix: Buffer, length: number) {
        const contextSum = await globalThis.crypto.subtle.digest('SHA-1', loginContext)
        ...
        while (true) {
            ...
            const sum = await globalThis.crypto.subtle.digest('SHA-1', Buffer.concat([prefix, Buffer.from(suffix)]))
